...
export default function FlagStatus({ flagStatus, hideBuild = false }) {
  return (
    <div
      className={`fw-bold d-inline-block lh-1 text-sm ${
    ...
    >
      {!hideBuild && <span className="d-inline-block bg-dark bg-opacity-25 h-100 px-2 py-1">{flagStatus.build}</span>}
      <span className="d-inline-block h-100 px-2 py-1">
        {flagStatus.status === "unknown" ? (
          ...
          <>
            E<span className="d-none d-lg-inline">nabled </span>b<span className="d-none d-lg-inline">y </span>D
            <span className="d-none d-lg-inline">efault</span>
          </>
        ) : flagStatus.status === "disabled-by-default" ? (
          <>
            D<span className="d-none d-lg-inline">isabled </span>b<span className="d-none d-lg-inline">y </span>D
            <span className="d-none d-lg-inline">efault</span>
          </>
        ) : flagStatus.status === "always-disabled" ? (
          <>
