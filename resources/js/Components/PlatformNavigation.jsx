...
export default function Navigation({ home = false, platforms, routeName, all = false }) {
  ...
  const mainPlatforms = useMemo(
    () =>
      platforms.filter(
        (platform) => (matchesSmUp && !platform.legacy) || (!matchesSmUp && !platform.legacy && !platform.tool),
      ),
    [matchesSmUp, platforms],
  );
  ...
  return (
    <nav className="navbar navbar-expand sticky-top">
      <div className="container">
        <div className="navbar-collapse collapse" id="navbar-page">
          ...
          <ul className="navbar-nav">
            <li className="nav-item dropdown">
              <a
                className={clsx("nav-link dropdown-toggle", { active: isActiveOverflow })}
                href="#"
                id="legacyPlatforms"
                role="button"
              ...
              >
                <span className="d-none d-sm-inline-block">
                  <Amicon icon={aiAngleDown} /> Legacy
                </span>
              ...
              <ul className="dropdown-menu dropdown-menu-end" aria-labelledby="legacyPlatforms">
                {!matchesSmUp && toolPlatforms.length > 0 && (
                  <>
                    ...
                    {toolPlatforms.map((platform, key) => (
                      <DropdownItem
                        url={route(routeName, { platform })}
                        active={route().current(routeName, platform)}
                        key={key}
                      >
                ...
                {!matchesSmUp && legacyPlatforms.length > 0 && (
                  <li>
                    <h6 className="dropdown-header">Legacy</h6>
                  </li>
                )}
