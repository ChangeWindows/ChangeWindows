...
export default function ReleaseCard({ name, platform, channels, alts, flight, url, dates, pack = false }) {
    ...
    <div className="col-12">
      <Component {...mainProps} className="card release">
        <div className="d-flex flex-row">
          {platform && <h3 className="h6 mb-0 d-flex align-items-center"><PlatformIcon platform={platform} color /></h3>}
            ...
            <div className="col-12 col-md-8">
              <div className="row">
                ...
                <div className="col-lg-4 col-12 d-flex flex-column align-items-start justify-content-center">
                  {!pack && flight && !platform?.tool && <small className="text-muted mb-0 mt-1 mt-lg-0">{flight}</small>}
                  {!pack && alts && !platform?.tool && <small className={clsx('text-muted mb-0 mt-lg-n1', { 'mt-n1': flight, 'mt-1': !flight })}>{alts.join(', ')}</small>}
                </div>
              </div>
            </div>
            {channels &&
