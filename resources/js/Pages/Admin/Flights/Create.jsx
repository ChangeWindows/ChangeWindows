...
export default function Create({ releases }) {
  ...
  useEffect(() => {
    setCurFlight((curFlight) => ({
      ...curFlight,
      major:
        !isNaN(version.split(".")[0]) && !!version.split(".")[0]
          ? Number(version.split(".")[0])
          : null,
      minor:
        !isNaN(version.split(".")[1]) && !!version.split(".")[1]
          ? Number(version.split(".")[1])
          : null,
      build:
        !isNaN(version.split(".")[2]) && !!version.split(".")[2]
          ? Number(version.split(".")[2])
          : null,
      delta:
        !isNaN(version.split(".")[3]) && !!version.split(".")[3]
          ? Number(version.split(".")[3])
          : null,
    }));
  }, [version]);
    ...
    return releases.filter((release) => {
      if (
        ...
        (Number(curFlight.build) < Number(release.start_build) ||
          (Number(curFlight.build) === Number(release.start_build) &&
            Number(curFlight.delta) < Number(release.start_delta)) ||
          Number(curFlight.build) > Number(release.end_build) ||
          (Number(curFlight.build) === Number(release.end_build) &&
            Number(curFlight.delta) > Number(release.end_delta)))
      ) {
        return false;
      }
    ...
    switch (name) {
      case "channel":
        if (_flight.releaseChannels.find((channelId) => channelId === id)) {
          _flight.releaseChannels = _flight.releaseChannels.filter(
        ...
        <div className="container my-3">
                    ...
                    <div className="col-12 col-sm-6">
                      <div className="form-floating">
                        <input
                          ...
                          id="date"
                          value={
                            isValid(parse(curFlight.date, "P", new Date()))
                              ? format(parseISO(curFlight.date), "yyyy-MM-dd")
                              : curFlight.date
                          }
          ...
          <fieldset className="row mb-3">
            <div className="col-12 col-md-4 my-4 my-md-0">
              ...
              <p className="text-muted mb-0">
                <small>
                  Socializing, but safe of course, it's still a pandemic...
                </small>
              </p>
            </div>
            <div className="col-12 col-md-8">
                        ...
                        <label className="form-check-label" htmlFor="tweet">
                          ...
                          <p className="lh-sm mt-1 mb-0">
                            <small className="text-muted d-block mt-n1">
                              Tweet to the platform-connected Twitter handles.
                            </small>
                          </p>
                        </label>
                      </div>
          ...
          <fieldset className="row mb-3">
            <div className="col-12 col-md-4 my-4 my-md-0">
              ...
              <p className="text-muted mb-0">
                <small>All channels this flight is in.</small>
              </p>
            </div>
            <div className="col-12 col-md-8">
              <div className="card">
                        ...
                        {release.availableChannels.map((channel, key) => (
                          <div className="form-check" key={key}>
                            <input
                              ...
                              id={channel.id}
                              name="channel"
                              checked={curFlight.releaseChannels.find(
