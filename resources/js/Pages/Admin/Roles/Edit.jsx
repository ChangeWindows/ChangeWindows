  ...
  return (
    <Admin>
      <form onSubmit={handleSubmit}>
        <NaviBar back="/admin/roles" actions={can.roles.edit && <SaveButton loading={processing} />}>
          {data.name || "Unnamed role"}
        </NaviBar>

        <div className="container my-3">
          <Status status={status} />
          <Fieldset title="General" description="Basic settings." disabled={!can.roles.edit}>
            <div className="col-sm-6 col-12">
              <TextField id="name" label="Name" value={data.name} errors={errors.name} onChange={setData} />
            </div>
          </Fieldset>
          <Fieldset title="Permissions" description="What this role can do." disabled={!can.roles.edit}>
            {permissions.map((permission, key) => (
              <div className="col-sm-6 col-md-4 col-12" key={key}>
                <Checkbox
                  id={permission.name}
                  name={permission.name}
                  label={permission.name}
                  checked={data.permissions.filter((_permission) => _permission === permission.name).length === 1}
                  disabled={data.permissions.find(
                    (_permission) => _permission === permission.name.substr(0, permission.name.indexOf(".")),
                  )}
